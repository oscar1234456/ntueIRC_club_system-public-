  ...
  <body>
    ...
    <?php
      try {
        ...
        $q2 = $conn ->query("select Carde_Name from Carde");
...
<div id="sidebar">
    <div id="content">
        <div class="title">指定幹部資料</div>      
            <div class="learn">
            
            <form action="carde_add_keep.php" method="post">
            <h4>請選擇您要指定的幹部:</h4>
              <div class="form-row">
                <div class="form-group col-md-8">
                  <select class="custom-select" name = "choose_Carde" required>
                    <option value=''>選擇幹部......</option>
                    <?php
                      if(handle_carde_display($rows2,"社長") == false){
                          print "<option value='社長'>社長</option>";
                      }
                      if(handle_carde_display($rows2,"副社長") == false){
                        print "<option value='副社長'>副社長</option>";
                      }
                      if(handle_carde_display($rows2,"總務") == false){
                        print "<option value='總務'>總務</option>";
                      }
                      if(handle_carde_display($rows2,"資訊管理") == false){
                        print "<option value='資訊管理'>資訊管理</option>";
                      }
                      if(handle_carde_display($rows2,"文書") == false){
                        print "<option value='文書'>文書</option>";
                      }
                  ?>
                  </select>
            ...
            <h4>請選擇您要指定職位的社員:</h4>
              <div class="form-row">
                <div class="form-group col-md-8">
                 ...
                 <!-- <input type="hidden" name = "M_Name" value = <?php print $rows[$i][1];?>>-->
                  <div class="invalid-feedback">Example invalid custom select feedback</div>
                </div>
              </div>
              <h4>請選擇開始日期:</h4>
              <div class="form-row">
                ...
                <input type="date" class="form-control" id="inputDate" name ="Begin_date" required>
                  <div class="invalid-feedback">Example invalid custom select feedback</div>
                </div>
              </div>
              ...
              <div class="form-row">
                <div class="form-group col-md-4">
                <input type="date" class="form-control" id="inputDate" name ="End_date" required>
                  <div class="invalid-feedback">Example invalid custom select feedback</div>
                </div>
              </div>
              <div class="form-row">
                <div class="form-group col-md-4">
                    <button class="btn btn-primary" type="submit">綁定</button>
                  </div>
              </div>
            </form>      
            </div>
