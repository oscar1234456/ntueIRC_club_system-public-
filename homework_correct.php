...
<html lang="en">
  ...
  <script>
     function express(HId){
       if(HId!=null)
          {window.open("uploadfile_query.php?HId="+HId, "newwindow", "height=400, width=500, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");}
      else{
          window.alert("請選擇其中一項作業查詢！");
        }
    }
    function display(){
      if(document.getElementById("choose_Member").value != 0){
        return document.getElementById("choose_Member").value;
      }else{
        return null;
      }
  ...
  <body>
    ...
    <?php
      try {
        ...
        $q = $conn ->query("select HId,Title from Homework");
        $rows = $q->fetchAll();
...
<div id="sidebar">
    <div id="content">
        <div class="title">批改作業</div>      
            <div class="learn">
            <h4>請選擇您要批改的作業:</h4>
            <form action="homework_correct_keep.php" method="post">
              <div class="form-row">
                <div class="form-group col-md-8">
                  <select class="custom-select"id = "choose_Member" name = "choose_Member" required>
                ...
                <div class="form-group col-md-4">
                  <button class="btn btn-primary" type="button" onclick="express(display())">查詢</button>
                </div>
                <div class="form-group col-md-4">
