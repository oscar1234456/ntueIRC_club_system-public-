  ...
  <body>
    ...
    <?php
      try {
        ...
        $q2 = $conn ->query("select SId,M_Name from Member where not exists(select * from Carde where SId = CSId);");
        $rows2 = $q2->fetchAll();
...
<div id="sidebar">
    <div id="content">
        <div class="title">刪除社員</div>      
            <div class="learn">
            <br>
            <h4>請選擇您要<strong><mark>刪除</mark></strong>的社員:</h4>
            <form name ="form1" action="delete_Member_keep.php" method="post">
              <div class="form-row">
            ...
            <p>請注意，您必須先至 幹部管理系統 ，<strong>解除社員職務</strong>，才可以刪除該名社員</p>
            <p>如需刪除社長資料，請先至 幹部管理系統 進行社長更換</p>
            <p>非必要，請不要刪除社員資料，應當修改社員資料 在籍狀態即可！</p>
          </div>
