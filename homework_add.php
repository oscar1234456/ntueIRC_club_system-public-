        ...
        <div class="title">新增作業</div>      
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST'){
              try {
                ...
                $q = $conn ->prepare("insert into Homework values(?,?,?,?,?,?)");
                $q ->execute(array($_POST['HId'],$_POST['Title'],$_POST['Homework_Start_date'],$_POST['Homework_End_date'],$_POST['Homework_content'],$_POST['HCId']));
                  print "已完成新增作業<br>";
                  print "即將返回作業管理畫面！";
                  ?>
              <script type="text/javascript"> setTimeout("window.location.href='homework_menu.php'",3000); </script>
             <?php }
              ...
              try {
                ...
                $q2 = $conn->query("select CId,Course_name,Teacher_Name from Course,Teacher where TeacherId = Teach_Id;");
                //$rows = $q -> fetchAll();
                $rows2 = $q2 -> fetchAll();
...
<form action="<?php $_SERVER['PHP_SELF']?>" method = "post" >
  <div class="form-row">
    <div class="form-group col-md-6">
      <label>作業編號</label>
      <input type="text" class="form-control" id="inputEmail4" name ="HId"placeholder="屆數2碼＋序號2碼" required>
    </div>
    <div class="form-group col-md-6">
    <label for="inputName">作業標題</label>
      <input type="text" class="form-control" id="inputName" name ="Title" placeholder="標題" required>
    </div>
  </div>
  <div class="form-row">
    <div class="form-group col-md-6">
      <label for="inputName">開始日期</label>
      <input type="date" class="form-control" name ="Homework_Start_date"id="inputBDate" required >
    </div>
    <div class="form-group col-md-6">
    <label for="inputBDate">結束日期</label>
    <input type="date" class="form-control" name ="Homework_End_date"id="inputBDate" required >
    </div>
  </div>
  
  
  <div class="form-group">
    <label for="inputFBName">作業內容</label>
    <input type="text" class="form-control" id="inputFBName"name ="Homework_content" placeholder="課程大綱">
  </div>
  <div class="form-row">
                <div class="form-group col-md-8">
                <label for="inputTeacher">課堂選擇</label>
                  <select class="custom-select" name = "HCId" required>
                    <option value="">選擇課堂......</option>
                    <?php
                        for($i=0;$i<$row2_count;$i++){
                          
                            print "<option value=".$rows2[$i][0].">".$rows2[$i][0]." ".$rows2[$i][1]." " .$rows2[$i][2]."</option>";
   ...
   <div class="form-group">
   <button class="btn btn-primary" type="submit">新增作業</button>
